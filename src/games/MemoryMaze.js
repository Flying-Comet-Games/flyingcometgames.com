...
const MemoryMaze = () => {
  ...
  const generatePath = () => {
    ...
    for (let i = 1; i < PATH_LENGTH; i++) {
      const possibleMoves = [
        ...
        [currentPosition[0], currentPosition[1] - 1],
      ].filter(([x, y]) => x >= 0 && x < GRID_SIZE && y >= 0 && y < GRID_SIZE && !newPath.some(p => p[0] === x && p[1] === y));

      if (possibleMoves.length === 0) break;

      const nextPosition = possibleMoves[Math.floor(Math.random() * possibleMoves.length)];
  ...
  const handleCellClick = (x, y) => {
    ...
    const cellIndex = playerPath.findIndex(pos => pos[0] === x && pos[1] === y);
    if (cellIndex !== -1) {
  ...
  const checkPath = (playerPath) => {
    ...
    const playerPathSet = new Set(playerPath.map(pos => `${pos[0]},${pos[1]}`));

    const correct = pathSet.size === playerPathSet.size &&
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 2 }}>
      <Typography variant="h4" sx={{ mb: 2 }}>Memory Maze</Typography>
      
      <Paper elevation={3} sx={{ p: 2, mb: 2 }}>
        <Typography variant="h6" sx={{ mb: 1 }}>Rules:</Typography>
        <Typography variant="body2" align="left">
          1. Memorize the path of {PATH_LENGTH} cells shown at the start of each round.<br/>
          2. Recreate the path by clicking on the cells (order doesn't matter).<br/>
          3. Click a cell again to deselect it.<br/>
          4. Click the Submit button when you've selected all {PATH_LENGTH} cells.<br/>
          5. You have {MAX_ATTEMPTS} attempts per round.<br/>
      ...
      <Grid container spacing={1} sx={{ width: 300, height: 300, margin: 'auto', mb: 2 }}>
        {maze.map((row, x) => 
          row.map((_, y) => (
            <Grid item xs={12 / GRID_SIZE} key={`${x}-${y}`}>
              <Box 
                ...
                sx={{ 
                  ...
                  paddingBottom: '100%', 
                  backgroundColor: 
                    (showPath && path.some(pos => pos[0] === x && pos[1] === y)) ?
              ...
              >
                {showPath && path.findIndex(pos => pos[0] === x && pos[1] === y) !== -1 && (
                  <Typography
                    sx={{
                      position: 'absolute',
                      top: '50%',
                  ...
                  >
                    {path.findIndex(pos => pos[0] === x && pos[1] === y) + 1}
                  </Typography>
                )}
              </Box>
            </Grid>
