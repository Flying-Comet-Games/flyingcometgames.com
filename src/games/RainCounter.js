...
const RainCounter = () => {
  ...
  const handleMatch = (matches) => {
    const now = Date.now();
    // Calculate new combo first
    const newCombo =
      lastMatchTime && now - lastMatchTime < COMBO_TIME_WINDOW
        ? Math.min(combo + 1, 5)
        : 1;

    // Calculate points using the new combo value
    ...
    setMessage(
      `+${points} points! ${newCombo > 1 ? `${newCombo}x Combo!` : ""}`
    );
    ...
    for (let col = 0; col < GRID_SIZE; col++) {
      ...
      for (let row = emptyRow; row >= 0; row--) {
        const shouldAddLightning =
          Math.random() < TILES.LIGHTNING.probability && combo > 2;
